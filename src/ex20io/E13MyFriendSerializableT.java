...
객체생성은 하지 않는다. 상속의 목적으로만 정의된 클래스이다.
 */
...
class HighFriend extends Friend {
	...
	/*
	고딩 친구의 전체정보 출력을 위해 부모에서 정의된 메소드를
...
class FriendInfoHandler {
	...
	private int numOfFriends;//친구정보를 추가할 때 마다 +1증가
	...
	2. High객체가 Friend타입으로 자동 형 변환되어 저장
	3. 객체배열에 저장된 객체들을 for문을 통해 순차적으로 출력
		이때 오버라이딩된 메소드를 통해 상상 해당 객체의
		정보를 출력 할 수 있음.(참조변수의 타입에 영향을
		받지 않음)
	 */
	...
	public void deleteInfo() {
		...
		/*
		배열의 요소 중 삭제된 요소의 인덱스 값을 저장 할 용도의 변수
		...
		if(deleteIndex == -1) {
			...
			/*
			객체배열에서 검색된 요소를 삭제한 후 입력 된 위치의 바로 뒤 요소를
			앞으로 하나씩 교환한다.
			 */
	...
	public void saveFriendInfo() {
		...
		try {
			...
			//myFriends 객체배열에 저장 된 친구의 정보만큼 반복
			for(int i = 0; i < numOfFriends; i++) {
				
				//객체배열의 i번째 요소를 파일로 저장함
