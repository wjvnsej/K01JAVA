...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		부모클래스의 참조변수가 자식클래스의 객체를 참조 할 수 있다.
		이 경우 접근범위는 부모클래스로 제한된다.
		즉 아래 호출 부분에서 speaker 참조변수를 통해서는 setBase()
		메소드는 호출 할 수 없다. setBase()는 자식클래스에 정의된
		멤버메소드이기 때문이다.
		단, 오버라이딩 처리된 메소드가 있는 경우 하위 클래스의 메소드가
		호출되게 된다.
		 */
