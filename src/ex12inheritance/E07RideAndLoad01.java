...
오버로딩 : 함수명은 같으나 매개변수의 갯수, 타입, 순서가
	...
	하위 클래스에는 오버로딩된 모든 메소드가 포함된다.
	
오버라이딩 : 함수명, 매개변수, 반환형까지 모두 동일한 함수를
	상속관계에 있는 하위 클래스에서 재정의 한 것을 말한다.
	이 경우 하위 클래스에서 정의한 메소드가 상위클래스의 메소드를
	가지게 되므로 항상 최하위 클래스의 메소드가 호출되게 된다.
...
////////////////////////////////////////////////////////
		...
		/*
		rdfId1은 A타입의 참조변수이므로 B타입 혹은 C타입의
		참조변수로 참조할 수 없다. 자식타입의 참조변수로
		부모객체를 참조하는 것은 불가능하다.
		 */
//		B refId2 = rdfId1;
//		C refId3 = rdfId1;
		//아래와 같이 다운캐스팅(강제 형변환)하면 참조 가능하다.
		C refId3 = (C)refId1;
