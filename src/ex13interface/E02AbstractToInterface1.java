...
- 객체생성은 할 수 없으나 그 회 참조변수, 상속 등의 모든 기능을
	가지고 있다.
	
추상메소드
- 오버라이딩의 목적으로 만들어진다
- 함수의 실행부가 없기때문에 {}를 쓰지않고 함수의 원형에 ;을 붙여준다.
 */

//프로젝트 전체에서 기준이 될 수 있는 추상클래스 정의
abstract class PersonalNumberStorageAbs {
	public abstract void addPersnalInfo(String juminNum,
			String name);
	public abstract String searchPersnalInfo(String juminNum);
}

/*
사람의 정보를 저장 할 용도의 DTO클래스. 혹은 VO클래스 라고도 한다.
 */
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {
	...
	/*
	전달받은 인자를 통해 PersonalInfoDTO 객체를 생성 후 객체배열에 저장한다.
	 */
	@Override
	public void addPersnalInfo(String juminNum, String name) {
	 ...
	 */
	@Override
	public String searchPersnalInfo(String juminNum) {
...
public class E02AbstractToInterface1 {
	...
	public static void main(String[] args) {
		...
		//정보저장
		storage.addPersnalInfo("901234-2222222", "김태희");
		storage.addPersnalInfo("901234-1111111", "정지훈");
		
		//검색
		System.out.println
			(storage.searchPersnalInfo("901234-2222222"));
		System.out.println
			(storage.searchPersnalInfo("901234-1111111"));
		System.out.println
			(storage.searchPersnalInfo("001024-1090333"));		
	}
}