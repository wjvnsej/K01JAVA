...
class MyClass {
	...
	/*
	toString() : 실제 저장된 데이터가 반환되도록 Object 클래스로 부터
		상속받아 오버라이딩한 메소드
	 */
	@Override
	...
	/*
	MyClass형 객체의 비교를 위해 오버라이딩한 메소드.
	객체가 가지고 있는 멤버변수의 비교를 통해 동일 여부를 판단한다.
	 */
	@Override
...
class Point {
	...
	/*
	객체 1, 2의 x,y값을 동시에 비교해서 둘 다 동일할 때 같은 객체로
	판단하도록 오버라이딩 처리
	 */
	@Override
	public boolean equals(Object obj) {
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		if(mc1 == mc2) {
			...
			System.out.println
				("인스턴스 참조값(주소값)이 다르다.");//[0]
		}
		
		//두 객체의 멤버변수 값 비교
		System.out.println
			(mc1.equals(mc2) ? "같다." : "다르다.");//[같다.]
		
		//toString()메소드에 오버라이딩 한 내용을 출력
		System.out.println("[두 객체의 toString()메소드 호출]");
		System.out.println
			("mc1.toString() =>" + mc1.toString());
		 ...
		 */
		System.out.println
			("pos1.toString() =>" + pos1.toString());
		
		System.out.println
			("pos2.toString() =>" + pos2.toString());
