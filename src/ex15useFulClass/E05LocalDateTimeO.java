...
public class E05LocalDateTimeO {
	...
	public static void main(String[] args) {
		...
		//특정 날짜 1995년 12월 14일
		LocalDate ofDate = LocalDate.of(1995, 12, 14);
		...
		//특정 날짜와 시간
		LocalDateTime ofDateTime = 
				LocalDateTime.of(1995, 12, 14, 13, 34, 33);
		...
		LocalDateTime day3Later = 
				LocalDateTime.now().plusDays(3).with
				(LocalTime.NOON);
		...
		LocalDate fourthSunday = LocalDate.of(2020, 3, 1).with
				(TemporalAdjusters.dayOfWeekInMonth
						(4, DayOfWeek.SUNDAY));
		System.out.println("2020년 3월 기준, 네 번째 일요일 : " + 
					fourthSunday);
		...
		LocalDate xmas = LocalDate.of
				(LocalDate.now().getYear(), 12, 25);
		Period left = Period.between(nowDate, xmas);
		System.out.println("X-mas까지 앞으로 " + left.getMonths() + 
				"개월 " + left.getDays() + "일");
