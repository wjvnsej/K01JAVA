...
public class E03AnonymousClass {
	...
	public static void main(String[] args) {
		...
		반드시 다운캐스팅 후 접근해야 한다.
		 */
		Person person = new Student("문사랑", "2016");
		person.printInfo();
//		person.stNumber = "2015";//[에러발생]
		...
		/*
		Person 클래스를 상속받은 익명클래스 정의 및 생성
...
//		anonyPerson.newMethod();//[접근불가 - 에러발생]
		/*
		부모타입의 참조변수로 하위객체에서 확장한 멤버에 접근할 수 
		없으므로 위 부분은 에러가 발생한다.
		 */
...
//		((Student)anonyStudent).age = 100;//다운캐스팅 불가 - 에러발생
		...
		형 변환 하여 하위클래스의 멤버에 접근하기 위한 수단이다.
		그러나 익명클래스는 클래스명이 없기때문에 위와 같이 다운캐스팅이
		불가능하다.
